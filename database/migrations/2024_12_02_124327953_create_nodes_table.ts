...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer('daemon_listen').unsigned().notNullable().defaultTo(8080)
      table.integer('daemon_sftp').unsigned().notNullable().defaultTo(2022)
      table.string('daemon_base').notNullable().defaultTo('/home/daemon-files')
