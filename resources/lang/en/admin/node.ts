export default {
  validation: {
    ...
    fqdn_required_for_ssl:
      'A fully qualified domain name that resolves to a public IP address is required in order to use SSL for this node.',
  },
  notices: {
    ...
    location_required:
      'You must have at least one location configured before you can add a node to this panel.',
    node_created:
      "Successfully created new node. You can automatically configure the daemon on this machine by visiting the 'Configuration' tab. <strong>Before you can add any servers you must first allocate at least one IP address and port.</strong>",
    node_updated:
